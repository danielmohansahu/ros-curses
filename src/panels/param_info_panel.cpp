...
ActionPacket ParamInfoPanel::render(const std::optional<ComputationalGraph>& graph)
{
  ...
  if (!graph || !_active_param || graph->params().find(*_active_param) == graph->params().end())
    return NULL_ACTION;
  
  // otherwise print some info about this topic
  const auto& param = graph->params().at(*_active_param);

  // header information
  size_t current_idx = 1;
  print_line(current_idx++, *_active_param + ((param->active) ? " (active)" : " (inactive)"));
